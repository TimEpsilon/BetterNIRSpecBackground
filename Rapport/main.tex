...
\usepackage[sorting=none, style=authoryear]{biblatex} %Imports biblatex package
...
\DeclareCiteCommand{\parenciteauthortitle}
  {\usebibmacro{prenote}}
  {(\printnames{labelname}, \printfield{title}\addperiod)}
  {\multicitedelim}
  {\usebibmacro{postnote}}

% Ensure the new citation is hyperlinked
\DeclareCiteCommand{\parenciteauthortitle}
  {\usebibmacro{prenote}}
  {(\hyperlink{\thefield{entrykey}}{\printnames{labelname}, \printfield{title}}\addperiod)}
  {\multicitedelim}
  {\usebibmacro{postnote}}
...
{\usebibmacro{prenote}}
{\hyperlink{\thefield{entrykey}}{(\printnames{labelname}, \printdate)}}
{\multicitedelim}
{\usebibmacro{postnote}}
...
Parmi les instruments du \gls{jwst}, \gls{nirspec} est un spectromètre sur la bande 0.6 - 5.3  µm, disposant de 3 modes de résolution : PRISM ($R \sim 100$), medium grating ($R \sim 1000$), high grating ($R \sim 2700$) \parenciteauthortitle{nirspec}. L'un des atouts principaux de \gls{nirspec} est son \gls{msa}, une grille de près de 250 000 obturateurs, couvrant une surface de 3.6' x 3.4' sur le ciel, chacun pouvant être individuellement ouvert ou fermé \parenciteauthortitle{msa} (voir \ref{fig:msa_shutter}). Ceci permet d'observer dans le mode \gls{mos} de \gls{nirspec}. Comme son nom l'indique, la spectroscopie multi-objet permet d'étudier le spectre de plusieurs objets dans un même champ, mais la grille d'obturateurs apporte également un outil fondamental dans l'extraction des spectres : la soustraction du fond.\\

Nous travaillerons ici sur des données \gls{nirspec} du Programme de \textit{The Cosmic Evolution Early Release Science (CEERS)}, disponibles sur la base de données MAST \parenciteauthortitle{portal_mast} (Proposal ID : 1345), notamment les observations P4, P5, P7, P8, P9, P11, P12, avec comme résolution PRISM.
